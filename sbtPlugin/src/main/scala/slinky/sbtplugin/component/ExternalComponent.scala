...
object ExternalComponent {
  ...
  def transform(obj: Defn.Object, scalaVersion: String): Defn.Object = {
    ...
    val methods                = createExternalMethods(stats, elementType, refType, scalaVersion)
  ...
  private def determineTypes(baseInit: Init): (Type, Type) =
    baseInit match {
      ...
      case Init(
          Type.Apply(Type.Name("ExternalComponentWithAttributes"), List(Type.Singleton(Term.Select(qual, tag)))),
          _,
          _
          ) =>
      ...
      case Init(
          Type.Name("ExternalComponentWithAttributesWithRefType"),
          _,
          List(List(Type.Select(qual1, name1), Type.Select(qual2, name2)))
          ) =>
      ...
      case Init(
          Type.Name("ExternalComponentWithAttributesWithRefType"),
          _,
          List(List(elemType: Type, refType: Type))
          ) =>
        (elemType, refType)

      case x @ _ =>
        throw new Exception("Invalid external component base class")
    }
  ...
  private def createExternalMethods(
    ...
    stats.flatMap {
      case q"case class Props[..$tparams](...$paramss) extends ..$_" =>
        // Preserve all parameter information including defaults
        ...
        if (paramss.flatten.forall(_.default.isDefined) || paramss.flatten.isEmpty) {
          Seq(
            basicApply,
            q"""
              def apply(mods: Seq[_root_.slinky.core.TagMod[$elementType]]):
